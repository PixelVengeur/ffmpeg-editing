...
if ($ourbals -eq 1)
{
    ...
    # Write-Host("videoLength = $videoLength,
    ...
    function generateTextBlurb {
        ...
        $filterComplete = "drawtext=text=${text}:fontfile=C\\:/Users/Nathan/AppData/Local/Microsoft/Windows/Fonts/OpenSans-Bold.ttf:fontcolor=white@0.9:fontsize=48:alpha='if(lt(t,$startTS),0,if(lt(t,$startTS + 0.5),(t-$startTS)/0.625,if(lt(t,$startTS + 5.5),0.8,if(lt(t,$startTS + 6),(0.5-(t-($startTS + 5.5)))/0.625,0))))':x=50:y=(h-text_h)/10*9"
    ...
    foreach ($line in Get-Content .\audios.txt)
    {
        ...
        $subtitleFilter += generateTextBlurb -startTS $($prevAudioLength + 4) -text $sousTitres.$counter
        $subtitleFilter+= ", "
